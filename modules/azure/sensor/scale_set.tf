resource "azurerm_linux_virtual_machine_scale_set" "sensor_scale_set" {
  ...
  # Azure rejects a VMSS whose health_probe_id is not yet "active" — i.e. not yet
  # referenced by an LB rule targeting the VMSS backend pool. Without this, the VMSS
  # create can race ahead of the rule and fail with CannotUseInactiveHealthProbe.
  depends_on = [azurerm_lb_rule.management_lb_rule]
