...
variable "license_key_file_path" {
  description = "Path to your Corelight sensor license key file. Optional if fleet_url is configured."
  sensitive   = true
  type        = string
  default     = ""
  ...
  validation {
    ...
    error_message = "Either license_key_file_path must be provided or fleet_url must be configured."
  }
}
...
variable "sensor_asg_auto_scale_policy_name" {
  description = "The name of the auto-scale group scale-out policy"
  type        = string
...
variable "sensor_asg_name" {
  ...
  type        = string
  default     = "corelight-sensor-asg"
}

variable "sensor_asg_min_size" {
  description = "Minimum number of sensor instances in the auto-scale group"
  type        = number
  default     = 1
}

variable "sensor_asg_max_size" {
  description = "Maximum number of sensor instances in the auto-scale group"
  type        = number
  default     = 5
}

variable "sensor_asg_desired_capacity" {
  description = "Initial desired number of sensor instances in the auto-scale group"
  type        = number
  default     = 1
}

variable "sensor_instance_name" {
  description = "The name tag applied to EC2 instances launched by the auto-scale group"
  type        = string
...
variable "sensor_management_security_group_description" {
  ...
  type        = string
  default     = "Security group for the sensor which allows SSH access"
}
