...
run "test_minimal_configuration" {
  ...
  assert {
    condition = try(
      yamldecode(base64decode(trimspace(split("\n", split("content: ", split("path: /etc/corelight/deployment-metadata.yaml", module.sensor_config.cloudinit_config.part[0].content)[1])[1])[0])))["deployment_metadata.terraform_module"] == "aws/sensor",
      false,
    )
